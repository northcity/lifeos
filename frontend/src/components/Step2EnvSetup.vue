  ...
  <div class="env-setup-panel">
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  <input 
                    ...
                    :style="{ '--percent': ((customMaxRounds - 1) / Math.max(autoGeneratedRounds - 1, 1)) * 100 + '%' }"
                  />
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">{{ $t('graph.systemDashboard') }}</span>
        <span class="log-id">{{ simulationId || $t('graph.noSimulation') }}</span>
      </div>
...
const customMaxRounds = ref(10)   // 免费版上限 10 轮
...
const autoGeneratedRounds = computed(() => {
  ...
  // 免费版截断至 FREE_MAX_ROUNDS
  return Math.min(Math.max(calculatedRounds, 1), FREE_MAX_ROUNDS)
})
