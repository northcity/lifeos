...
logger = get_logger('lifeos.oasis_profile')
...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "你是个人人生模拟角色生成专家。为人生路径模拟生成真实、具体的 Agent 人设，用于预测个人财务/职业/家庭未来轨迹，最大程度还原已有现实情况。必须返回有效的JSON格式，所有字符串值不能包含未转义的换行符。"
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""为实体生成详细的人生模拟 Agent 人设，最大程度还原已有现实情况。
...
1. bio: 个人简介，200字，描述这个 Agent 在人生模拟中的个人简介
2. persona: 详细 Agent 人设描述（2000字的纯文本），需包含:
   - 基本信息（年龄、职业、所在地、收入水平、家庭状况）
   - 人物背景（重要经历、财务历史、职业发展轨迹、与用户的关系）
   - 性格特征（风险偏好、决策风格、生活态度）
   - 财务行为（消费编好、投资习惯、对金钱的态度）
   - 职业趋向（对工作的感受、设定职业目标的可能性、赞同/反对用户跳槽）
   - 对用户目标的影响力（支持/阻力/中立）
   - 个人记忆（与用户互动的平时记忆、与用户共同经历的事件）
3. age: 年龄数字（必须是整数）
4. gender: 性别，必须是英文: "male" 或 "female"
5. mbti: MBTI类型（如INTJ、ENFP等）
6. country: 国家（使用中文，如"中国"）
7. profession: 职业
8. interested_topics: 关注话题数组（财务/职业/家庭相关话题）
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""为机构/环境实体生成详细的人生模拟环境 Agent 设定，最大程度还原已有现实情况。
...
1. bio: 环境/机构简介，200字，描述其在人生模拟中扮演的角色
2. persona: 详细 Agent 设定描述（2000字的纯文本），需包含:
   - 机构/环境基本信息（正式名称、性质、主要职能）
   - 在用户人生中扮演的角色（对用户财务/职业/家庭的影响力）
   - 行为模式（如何影响用户的决策，支持/阻力/中立）
   - 对用户目标的态度（赞同跳槽/推荐留下/中立等）
   - 内在逻辑（市场环境如何变动，与用户事项的关联）
   - 环境记忆（与用户历史上的关联事件）
3. age: 固定填30（机构/环境 Agent 的虚拟年龄）
4. gender: 固定填"other"（机构账号使用other表示非个人）
5. mbti: MBTI类型，用于描述环境风格和行为特征
6. country: 国家（使用中文，如"中国"）
7. profession: 机构职能描述
8. interested_topics: 关注领域数组（与用户人生影响相关的领域）
