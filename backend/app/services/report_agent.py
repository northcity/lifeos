...
【用户个人档案与目标】
用户的三年目标与模拟需求：{simulation_requirement}

【人生模拟世界规模】
- 人生关键实体数量: {total_nodes}
- 实体间关系数量: {total_edges}
- 实体类型（财务/职业/家庭/市场等）: {entity_types}
- 参与模拟的人生 Agent 数量: {total_entities}

【模拟推演到的部分人生事实样本】
{related_facts_json}

请以「上帝视角」审视这个个人人生的预演：
1. 在设定的生活轨迹下，未来 3 年个人财务/职业/家庭预计将如何演变？
2. 稳健版自我和冲刺版自我两条路径各有强弱？
3. 关键风险和机会窗口在哪里？

根据个人人生预测结果，设计最合适的报告章节结构。建议包含：财务轨迹、职业路径、家庭里程碑、核心风险、决策建议。

【再次提醒】报告章节数量：最少2个，最多5个，内容要精炼聚焦于个人财务/职业/家庭的核心预测。"""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
你是一个「个人人生路径预测报告」的撰写专家，正在撰写报告的一个章节。

报告标题: {report_title}
报告摘要: {report_summary}
用户三年目标与模拟需求: {simulation_requirement}
...
人生模拟世界是对用户个人未来路径的预演。我们基于用户的真实现状，
构建了稳健版自我、冲刺版自我、配偶、职场和市场环境等人生 Agent，
模拟中 Agent 的互动，就是对用户个人财务/职业/家庭轨迹的预测。

你的任务是：
- 揭示在设定生活轨迹下，用户的财务/职业/家庭未来将如何演变
- 对比稳健路径与冲刺路径的利弊与关键节点
- 发现关键决策节点、风险和机会窗口，给出具体可执行的建议

❌ 不要写成泛泛的通识性建议
✅ 要基于模拟数据，给出具体的个人财务/职业/家庭预测内容
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                title="个人人生路径预测报告",
                summary="基于人生模拟的未来 3 年财务、职业与家庭轨迹预测",
                sections=[
                    ReportSection(title="财务轨迹预测"),
                    ReportSection(title="职业路径分析"),
                    ReportSection(title="核心风险与决策建议")
                ]
            )
